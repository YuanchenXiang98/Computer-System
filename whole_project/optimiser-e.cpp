...
bool is_call_from_term = false;

bool is_call_exist(ast t)
{
    ast term = get_term_term(t);
    ast_kind my_astkind = ast_node_kind(term);

    if (my_astkind == ast_call_as_function || my_astkind == ast_call_as_method)
    {
        return true;
    }
    else if (my_astkind == ast_expr)
    {
        int length = size_of_expr(term);

        for (int i = 0; i < length; i+=2)
        {
            ast_kind local_my_astkind = ast_node_kind(get_term_term(get_expr(term, i)));

            if (local_my_astkind == ast_call_as_function || local_my_astkind == ast_call_as_method)
            {
                return true;
            }
            if (local_my_astkind == ast_expr)
            {

                if (is_call_exist(get_expr(term, i)))
                {
                    return true;
                }
            }
...
bool is_real_constant(ast t)
{
    ast term = get_term_term(t);

    if (ast_node_kind(term) == ast_int || ast_node_kind(term) == ast_bool)
    {
        return true;
    }
    if (ast_node_kind(term) == ast_unary_op)
    {
        return is_real_constant(get_unary_op_term(term));
    }
    return false;
}
...
{
    ast term = get_term_term(t);
    if (ast_node_kind(term) == ast_int)
    {
        return get_int_constant(term);
    }
    if (ast_node_kind(term) == ast_bool)
    {
        if (get_bool_t_or_f(term) == true)
        {
            // true
            return -1;
        }
    ...
    if (ast_node_kind(term) == ast_unary_op)
    {
        string unary_op = get_unary_op_op(term);
        if (unary_op == "-")
        {
            return -get_value(get_unary_op_term(term));
        }
        if (unary_op == "~")
        {
            return ~get_value(get_unary_op_term(term));
...
static ast optimise_expr(ast t)
{
    ...
    if (size == 1)
    {
        if (ast_node_kind(get_term_term(get_expr(t, 0))) == ast_expr)
        {
            return optimise_expr(get_term_term(get_expr(t, 0)));
        }
    }
    bool local_op_term = true;
    for ( int i = 0 ; i < size ; i++ )
    {
        ast termop = get_expr(t,i) ;
        if (i % 2 == 0)
        {
            if (is_call_exist(termop) == true)
            {
                local_op_term = false;
            }
        }
    ...
    ast combined = create_empty();
    if (size == 3 && local_op_term == true)
    {
        ast left_hand = get_expr(t, 0);
        ast right_hand = get_expr(t, 2);
        string op = get_infix_op_op(get_expr(t, 1));

        if (ast_node_kind(get_term_term(left_hand)) == ast_expr) {
			left_hand = optimise_term(left_hand);
		}
		if (ast_node_kind(get_term_term(right_hand)) == ast_expr) {
			right_hand = optimise_term(right_hand);
		}
		if (is_real_constant(left_hand) == true && is_real_constant(right_hand) == true)
		{

		    if (op == "+")
		    {
		        combined = create_int(get_value(left_hand) + get_value(right_hand));
		    }
		    ...
		    else if (op == "/" && get_value(right_hand) != 0)
		    {
		        combined = create_int(get_value(left_hand) / get_value(right_hand));
		    }
		    ...
		    {
		        int num_combined = get_value(left_hand) & get_value(right_hand);
		        if (num_combined == 0)
		        {
		            combined = create_bool(false);
		        }
		        else if (num_combined == -1)
		        {
		            combined = create_bool(true);
		        }
		        else
		        {
		            combined = create_int(num_combined);
		        }
		    }
		    ...
		    {
		        combined = create_bool(get_value(left_hand) < get_value(right_hand));
		    }
		...
		else if (is_real_constant(left_hand) == true)
		{
		    if (get_value(left_hand) == 0)
		    {
		        if (op == "+")
		        {
		            combined = get_term_term(right_hand);
		        }

		        else if (op == "*")
		        {
		            combined = create_int(0);
		        }
		        else if (op == "&")
		        {
		            combined = create_bool(false);
		    ...
		    else if (get_value(left_hand) == 1 && op == "*")
		    {
		        combined = get_term_term(right_hand);
		    }
		...
		else if (is_real_constant(right_hand) == true)
		{
		    if (get_value(right_hand) == 0)
		    {
		        if (op == "-" || op == "+")
		        {
		            combined = get_term_term(left_hand);
		        }
		        else if (op == "*")
		        {
		            combined = create_int(0);
		    ...
		    else if (get_value(right_hand) == 1)
		    {
		        if (op == "*" || op == "/")
		        {
		            combined = get_term_term(left_hand);
		...
		else if (ast_node_kind(get_term_term(left_hand)) == ast_var && ast_node_kind(get_term_term(right_hand)) == ast_var)
		{
		    if (get_var_name(get_term_term(left_hand)) == get_var_name(get_term_term(right_hand)))
		    {
		        if (op == "-")
		        {
		            combined = create_int(0);
    ...
    if (ast_node_kind(combined) != ast_empty)
    {
        if (is_call_from_term == true)
        {
            return combined;
        }
        vector<ast> new_terms;
        new_terms.push_back(create_term(combined));
        return create_expr(get_ann(t), new_terms);
    }
...
static ast optimise_term(ast t)
{
    ...
    case ast_expr:
        ...
        {
            return optimise_term(get_expr(term, 0));
        }
        is_call_from_term = true;
        term = optimise_expr(term) ;
        is_call_from_term = false;
        break ;
    case ast_unary_op:
        if (ast_node_kind(get_term_term(get_unary_op_term(term))) == ast_unary_op)
        {
            if (get_unary_op_op(term) == get_unary_op_op(get_term_term(get_unary_op_term(term))))
            {
                return optimise_term(get_unary_op_term(get_term_term(get_unary_op_term(term))));
            }
        }
