...
static ast parse_static_var_dec()
{
    ...
    var_dec.push_back(declare_variable(name, type, "static"));
    ...
    mustbe(tk_semi);
    ast ret = create_var_decs(var_dec) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_field_var_decs()
{
    ...
    while (have(tk_field))
    {
        field_var_decs.push_back(parse_field_var_dec());
    }
    ast ret = create_class_var_decs(field_var_decs) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_field_var_dec()
{
    ...
    Token type = mustbe(tg_type);
    Token name = mustbe(tk_identifier);
    field_var_dec.push_back(declare_variable(name, type, "this"));
    ...
    mustbe(tk_semi);
    ast ret = create_var_decs(field_var_dec) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_subr_decs()
{
    ...
    while (have(tg_subroutine))
    {
        ...
        {
            is_constructor = true;
            subr_decs.push_back(create_subr(parse_constructor()));
        }
        else if (have(tk_function))
        {
            subr_decs.push_back(create_subr(parse_function()));
        }
        else if (have(tk_method))
        {
            subr_decs.push_back(create_subr(parse_method()));
        }
    }
    ast ret = create_subr_decs(subr_decs) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_param_list()
{
    ...
    // add code here ...
    vector<ast> params_list;

    if (have(tg_type))
    {
        Token type = mustbe(tg_type);
        Token name = mustbe(tk_identifier);
        params_list.push_back(declare_variable(name, type, "argument"));
    ...
    ast ret = create_param_list(params_list);
    pop_error_context() ;
    return ret ;
}
...
static ast parse_var_decs()
{
    ...
    while (have(tk_var))
    {
        ast_var_decs.push_back(parse_var_dec());
    }

    ast ret = create_var_decs(ast_var_decs) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_var_dec()
{
    ...
    Token type = mustbe(tg_type);
    Token name = mustbe(tk_identifier);
    ast_var_decs.push_back(declare_variable(name, type,"local"));
...
static ast parse_let()
{
    ...
    if (ast_node_kind(index) != ast_empty)
    {
...
static ast parse_void_var_call()
{
    ...
    if (have(tk_fn))
    {
        ...
        class_name = token_spelling(name);
    }
    ...
    else if (have(tk_stop))
    {
        ...
        class_name = get_var_type(object);
        have_object = true;
    }
...
static ast parse_void_this_call()
{
    ...
    ast ret = create_call_as_method(myclassname, object, subr_call) ;
    pop_error_context() ;
    return ret ;
}
...
static ast parse_return()
{
    ...
    ast expr = create_empty();

    if (have(tg_term))
    {

        if (is_return_empty == false && is_Void == true)
        {
        ...
        if (is_constructor == true)
        {
            mustbe(tk_this);
            mustbe(tk_semi);
            ast ret = create_return_expr(create_expr(create_term(create_this())));
            return ret;
        }
    ...
    if (have_expr == false)
    {
        ...
        if (is_return_empty == true && is_Void == false)
        {
    ...
    else
    {
        ...
        // if (is_constructor = true)
        // {
        //     ret = create_return_expr(mustbe(tk_this));
        // }
...
static ast parse_expr()
{
    ...
    // add code here ...
    ast term = parse_term();
    ast lhs = create_empty();
    ast rhs = create_empty();
    ast op = create_empty();
    ast ret = create_empty();
...
static ast parse_var_term()
{
    ...
    // add code here ...
    Token object = mustbe(tk_identifier);
    ast ret = create_empty() ;

    if (have(tk_lsb)) // need look_v_fatal
    {
        ast var = lookup_variable_fatal(object);
    ...
    {
        string class_name = token_spelling(object);
        ast subr_call = parse_fn_call();
        ret = create_call_as_function(class_name, subr_call);
    }

    else if (have(tk_stop)) // need_look_v_fatal
    {
        ast var = lookup_variable_fatal(object);
        ast subr_call = parse_method_call();
        ret = create_call_as_method(get_var_type(var), var, subr_call);
    }

    else // need_look_v_fatal
    {
        ret = lookup_variable_fatal(object);
    }
